...
export class ConversationRepository {
  ...
  async getConversationRecipientPhone(conversationId) {
    const conversation = mockConversations.find((entry) => entry.id === conversationId);
    if (!conversation) {
      throw new Error(`Conversation not found: ${conversationId}`);
    }
    if (typeof conversation.userId === 'string' && conversation.userId.startsWith('+')) {
      return conversation.userId;
    }
    throw new Error('Mock repository does not contain a real recipient phone number');
  }
