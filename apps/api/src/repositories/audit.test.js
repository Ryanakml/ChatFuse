...
import { logAuditEvent, getAuditEvents, setAuditClient } from './audit.js';
// ---- In-memory audit store ----
const auditStore = [];
const makeSelectChain = (filterFn = () => true) => ({
    order: () => ({
        limit: () => ({ data: auditStore.filter(filterFn), error: null }),
        data: auditStore.filter(filterFn),
        error: null,
    }),
    eq: (field, value) => makeSelectChain((e) => e[field] === value),
