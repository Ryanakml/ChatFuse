...
let failed = 0;
const getObservabilityEvents = (events, type) => events.filter((event) => event.type === type);
...
try {
    ...
    await runTest('POST /webhook rejects missing signature header', async () => {
        await withServer({}, async (baseUrl, _env, deps) => {
            ...
            const verificationFailureEvents = getObservabilityEvents(deps.observabilityEvents, 'verification_failure');
