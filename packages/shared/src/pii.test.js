...
import { maskPii, maskString, looksLikePhone, looksLikeEmail, MASKED_FIELD, MASKED_PHONE, MASKED_EMAIL, } from './pii.js';
...
describe('maskPii', () => {
    ...
    it('masks PII patterns in non-blocklisted string values', () => {
        ...
        const result = maskPii(input);
        expect(result['description']).toContain(MASKED_EMAIL);
        expect(result['description']).not.toContain('john@example.com');
    });
